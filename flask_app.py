...
def get_high_qual_seqs(filter,uploaded_files, num_start, num_end, rc, enter_qual_cutoff, rounds):
    ...
    for round in range(len(all_high_qual_seqs)):
        all_trimmed_seqs.append([w[num_start:num_end] for w in all_high_qual_seqs[round]])
    all_high_qual_seqs = all_trimmed_seqs

    orig_trimmed_seqs = []
    for round in range(len(origonal_seqs)):
        orig_trimmed_seqs.append([w[num_start:num_end] for w in origonal_seqs[round]])
...
def get_high_qual_seqs_NOTUNIQUE(filter,uploaded_files, num_start, num_end, rc, enter_qual_cutoff, rounds):
    return get_high_qual_seqs(filter,uploaded_files, num_start, num_end, rc, enter_qual_cutoff, rounds)
...
def choose_rounds():
    if request.method == 'POST':
        ...
        shared_dict["times"] = ''
        shared_dict["all_regions_all_seqs_with_region"] = []
    ...
    if shared_dict["size_to_upload"] == 'large':
            ...
            if shared_dict["num_rounds"]>1:
                if files_on != shared_dict["num_rounds"]:
                    ...
                    shared_dict["all_high_qual_seqs"], shared_dict["origonal_seqs"] = get_high_qual_seqs(shared_dict["qual_yes"],shared_dict["uploaded_files"], 0, 10000, 0, shared_dict["quality_cutoff"], range(shared_dict["num_rounds"]))
...
def preview():
    if request.method == 'POST':
        ...
        shared_dict["all_high_qual_seqs"], shared_dict["origonal_seqs"] = get_high_qual_seqs(shared_dict["qual_yes"],shared_dict["uploaded_files"], shared_dict["num_start"], shared_dict["num_end"], shared_dict["rc"], shared_dict["quality_cutoff"], range(shared_dict["num_rounds"]))
...
def most_abundant():
    if request.method == 'POST':
        ...
        high_qual_seqs_not_unique, origonal_seqs_not_unique = get_high_qual_seqs(shared_dict["qual_yes"],shared_dict["uploaded_files"], shared_dict["num_start"], shared_dict["num_end"], shared_dict["rc"], shared_dict["quality_cutoff"], range(shared_dict["num_rounds"]))
...
def download_abund():
    if request.method == 'POST':
        da_start = time.time()
        input_data = most_abundant_seqs
        output_data = ''
        output_data+='Sequence,Reads\n'
        for line in range(1000):
                output_data+=str(most_abundant_seqs[line][0]).strip('\n')+','+str(most_abundant_seqs[line][1]).strip('\n')+'\n'
...
def track_seqs():
    ...
    if request.method == 'POST':
        ...
        all_tracking_counts = []
        seq = 1
        for seq in range(len(counts_from_prior_rounds[0])):
            tn_log = []
            tn_counts = []
...
def region_emergence():
    if request.method == 'POST':
        ...
        for reg in all_regions:
            ...
            shared_dict["all_regions_all_seqs_with_region"].append(all_seqs_with_region)
...
def download_qual0():
    if request.method == 'POST':
        ...
        response.headers["Content-Disposition"] = "attachment; filename=high_quality.csv"
        return response
...
def download_clusters0():
    if request.method == 'POST':
        ...
        used = []
        range_max = min(len(shared_dict["all_labels"][round_num]), 100)
...
def download_region0():
    if request.method == 'POST':
        round_num = 0
        input_data = shared_dict["all_regions_all_seqs_with_region"][chosen_region][round_num]
        output_data = ''
        output_data+='Sequence,Reads\n'
        for line in range(1000):
            output_data+=str(input_data[line].strip('\n'))+','+str(shared_dict["origonal_seqs"][round_num].count(input_data[line+"\n"]))+'\n'
        response = make_response(output_data)
        response.headers["Content-Disposition"] = "attachment; filename=region_emergence.csv"
        return response
    if request.method == "GET":
...
def download_region1():
    if request.method == 'POST':
        round_num = 1
        input_data = all_seqs_with_region[round_num]
        output_data = ''
        output_data+='Sequence,Reads\n'
        for line in range(1000):
            output_data+=str(input_data[line].strip('\n'))+','+str(shared_dict["origonal_seqs"][round_num].count(input_data[line+"\n"]))+'\n'
